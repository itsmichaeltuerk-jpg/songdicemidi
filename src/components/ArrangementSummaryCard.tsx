...
export const ArrangementSummaryCard: React.FC<ArrangementSummaryCardProps> = ({
  ...
  return (
    <div className="w-full bg-gradient-to-b from-[#1A1C23] via-[#15171C] to-[#121316] rounded-3xl border border-white/10 p-5 sm:p-6 shadow-2xl space-y-4">
      {/* Top Banner: Title + Key/BPM Badges + Actions */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-white/10">
        <div className="space-y-1.5">
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-[10px] uppercase font-bold tracking-widest px-2.5 py-0.5 rounded-full bg-amber-500/10 text-amber-300 border border-amber-500/20 font-mono">
              ARRANGED BRIEF
            </span>
            <span className="text-zinc-500 text-xs">•</span>
            <button
              onClick={handleCopySeed}
              className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-lg bg-zinc-900 border border-white/10 hover:border-amber-400/40 text-amber-300 text-xs font-mono transition-all"
        ...
        <div className="flex items-center gap-2 flex-wrap shrink-0">
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-zinc-900 border border-white/10 text-xs font-mono font-bold text-amber-300">
            <Music className="w-3.5 h-3.5 text-amber-400" />
            <span>{arrangement.key}</span>
            <span className="text-zinc-500">({arrangement.mode})</span>
          </div>

          <div className="px-3 py-1.5 rounded-xl bg-zinc-900 border border-white/10 text-xs font-mono font-bold text-zinc-200">
          ...
          <button
            ...
            className={`p-2 rounded-xl border transition-all ${
              isFavorite
                ...
                : 'bg-zinc-900 border-white/10 text-zinc-400 hover:text-rose-400'
            }`}
        ...
        {arrangement.next_moves && arrangement.next_moves.length > 0 && (
          <div className="p-4 bg-zinc-950/70 rounded-2xl border border-white/5 space-y-2">
            <div className="flex items-center gap-2 text-emerald-400 text-xs font-bold font-mono uppercase tracking-wider">
              ...
              <span>Next Steps in Your DAW</span>
            </div>
        ...
        <div className="flex items-center gap-2 pt-1 overflow-x-auto pb-1 text-xs">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            {arrangement.form.map((section, idx) => (
              <span
                key={idx}
                className="px-2.5 py-1 rounded-lg bg-zinc-900/90 border border-white/10 text-zinc-300 font-mono text-[11px] font-medium"
              >
                {section}
              </span>
            ))}
